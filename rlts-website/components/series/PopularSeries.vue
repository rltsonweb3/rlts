<template>
    <div>
        ...
        <div class="awardedmovie-area relative">
            <Swiper :options="popularSeriesOption">
                <Swiper-Slide v-for="(product, Index) in popularSeries" :key="Index">
                    <SingleProduct 
                        :product="product" 
                        :PageLink="'/series'"
                        :btnLink="'/series'"
...
export default {
    ...
    mounted () {
        this.popularSeries = this.seriesData.filter(product=> product.popular == true)
    }
}
</script>
