<template>
    <div>
        ...
        <div class="awardedmovie-area relative">
            <Swiper :options="trendingSeriesOption">
                <Swiper-Slide v-for="(product, Index) in trendingSeries" :key="Index">
                    <SingleProduct 
                        :product="product" 
                        :PageLink="'/series'"
                        :btnLink="'/series'"
...
export default {
    ...
    mounted () {
        this.trendingSeries = this.seriesData.filter(product=> product.trending == true)
    }
}
</script>
